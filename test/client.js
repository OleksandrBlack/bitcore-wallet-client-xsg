...
var BWS = require('bitcore-wallet-service-safe');
...
helpers.toSatoshi = function(safe) {
  if (_.isArray(safe)) {
    return _.map(safe, helpers.toSatoshi);
  } else {
    return parseFloat((safe * 1e8).toPrecision(12));
  }
};
...
describe('client API', function() {
  ...
  describe('Build & sign txs', function() {
    ...
    describe('#signTxp', function() {
      ...
      it('should sign safe proposal correctly', function() {
        var toAddress = 'msj42CCGruhRsFrGATiUuh25dtxYtnpbTx';
        var changeAddress = 'msj42CCGruhRsFrGATiUuh25dtxYtnpbTx';
  ...
  describe('Network fees', function() {
    it('should get current fee levels for SAFE', function(done) {
      ...
      clients[0].credentials = {};
      clients[0].getFeeLevels('safe', 'livenet', function(err, levels) {
        should.not.exist(err);
        should.exist(levels);
  ...
  describe('Transaction Proposal signing', function() {
    ...
    describe('SAFE', function(done) {
      beforeEach(function(done) {
        setup(2, 3, 'safe', 'testnet', done);
      });
  ...
  var addrMap = {
    safe: ['1PuKMvRFfwbLXyEPXZzkGi111gMUCs6uE3','1GG3JQikGC7wxstyavUBDoCJ66bWLLENZC']
  };
